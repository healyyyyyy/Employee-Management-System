...
class Remove_Employee implements ActionListener{
    ...
    Remove_Employee(){
        ...
        // Create base frame
        f=new JFrame("Pearson Specter Litt - Remove Employee");
        ...
        l5.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("EMS/images/remove.jpg")); // background image from local file
        l5.setIcon(img);
        f.add(l5);
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b){
            try{
                ...
                String str = "select name,phone,email from employee where emp_id ='"+t.getText()+"' ";  // acquire employee info from database
                ResultSet rs = con.s.executeQuery(str);

                int i=0;
                if(rs.next()){                                                                          // get value in next column
                ...
                if(i==0)
                    JOptionPane.showMessageDialog(null,"Id not found");                                 // display error if invalid ID
            }catch(Exception ex){}
        }
        if(ae.getSource()==b1){
            try{
                conn con = new conn();
                String str = "delete from employee where emp_id = '"+t.getText()+"'";                   // delete record from database
        ...
        if(ae.getSource()==b2){                                                                         // go back to default landing screen
            f.setVisible(false);
    ...
    public static void main(String[]ar){
        new Remove_Employee();                                                                          // default call to frame
    }
}
