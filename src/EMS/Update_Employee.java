...
class Update_Employee extends Add_Employee implements ActionListener{
    ...
    Update_Employee(String idaa){
        ...
        super(0);                                                                               // reference variable to refer to objects in parent class
        ...
        id15.setLayout(null);
        ImageIcon img=new ImageIcon(ClassLoader.getSystemResource("EMS/images/update.jpg"));    // background image from local file
        id15.setIcon(img);

        id8 = new JLabel("Update Records:");
    ...
    void showData(String s){
        try{
            conn con = new conn();
            String str = "select * from employee where emp_id = '"+s+"'";                               // query to get employee info from database
            ResultSet rs = con.s.executeQuery(str);

            if(rs.next()){                                                                              // populate text fields from databse columns
                f.setVisible(true);
                i=1;
            ...
            if(i==0)
                JOptionPane.showMessageDialog(null,"Id not found");                                      // error message in case of invalid ID entered
            new Search_Employee();
        }catch(Exception ex){}
        f.setVisible(true);
        f.setSize(900,500);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b && i==1){
            try{
                ...
                // update emplyee table columns from data input in text fields
                String str = "update employee set name='"+t1.getText()+"',supervisor='"+t2.getText()+"',age='"+age+"',dob='"+dat+"',address='"+t3.getText()+"',phone='"+t4.getText()+"',email='"+t5.getText()+"',education='"+t6.getText()+"',post='"+t7.getText()+"',cnic='"+t8.getText()+"',emp_id='"+t9.getText()+"' where emp_id='"+id_emp+"'";
                con.s.executeUpdate(str);
                JOptionPane.showMessageDialog(null,"successfully updated");                             // success message
                f.setVisible(false);
                new Search_Employee();                                                                  // call search function
            }catch(Exception e){
                System.out.println("The error is:"+e);
            }
        }
        if(ae.getSource()==b1){                                                                         // go back to default landing page (logged in)
            f.setVisible(false);
    ...
    public static void main(String[] arg){
        new Update_Employee("Update Employee");                                                         // default call to frame
    }
}